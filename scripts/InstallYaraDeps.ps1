...
function Install-YARA {
    ...
    # Download Yara Rules
    $yaraRulesUrl = "https://github.com/whiterabb17/ossec-yara/raw/refs/heads/main/rules/yara_rules_v0.yar"
...
$yaraRulesPath = "C:\Program Files (x86)\ossec-agent\active-response\bin\yara\rules\yara_rules.yar"    
if (Test-Path -Path $yaraRulesPath) {
    ...
    InfoMessage "YARA rules downloaded successfully!" 
} else {
    ...
    InfoMessage "Yara Scanner Downloaded and copied into $yaraBatDir "

    # Update Sentury agent configuration
    Update-SenturyConfig
...
# Function to update Sentury agent configuration
function Update-SenturyConfig {
    ...
    if ($null -ne $syscheckNode) {
        ...
        if ($null -eq $existingNode) {
            ...
            InfoMessage "Common Downloads Directory added to syscheck configuration."
        } else {
            InfoMessage "Common Downloads Directory is already in the syscheck configuration."
        }

        $existingNode2 = $syscheckNode.directories | Where-Object { $_.InnerText -eq "C:\Windows\Temp" }
        if ($null -eq $existingNode2) {
            $newDirectoryNode = $configXml.CreateElement("directories")
            $newDirectoryNode.SetAttribute("realtime", "yes")
            $newDirectoryNode.InnerText = "C:\Windows\Temp"
            $syscheckNode.AppendChild($newDirectoryNode) | Out-Null
            $configXml.Save($configFilePath)
            InfoMessage "System Temp Folder added to syscheck configuration."
        } else {
            InfoMessage "System Temp Folder is already in the syscheck configuration."
        }

        $existingFileLimitNode = $syscheckNode.SelectSingleNode("file_limit") 

        if ($existingFileLimitNode -eq $null) {
            InfoMessage "Adding file_limit to Sentury agent configuration..." 
            try {
    ...
    if ($frequencyNode) {
        ...
        SuccessMessage "Frequency updated successfully in Sentury agent configuration file."
    } else {
